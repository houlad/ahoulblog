...
to see how the data is structured. (Note: Initial cleaning was performed in the [read_data.R](https://github.com/houlad/Yelp/blob/master/read_data.R) script which
...
restaurants <- readRDS(here::here("content", "post", 
                                  "Data", 'us_restaurants.rds'))
sample_reviews <- readRDS(here::here("content", "post", 
                                     "Data", 'sample_reviews_large.rds'))
...
with a similar variance at the low end. Make sure you use the same seed I did if you want to replicate 
the plot.
...
word_per_review <- readRDS(here::here("content", "post",
                                      "Data", 'review_words.rds'))
...
filter down to reviews for a couple of the highest variance (Fast Food and Chicken Wings) and a couple
...
This initial exploration is part one of my Yelp dataset series. As always, the full code 
for this analysis can be found [here](https://github.com/houlad/ahoulblog/blob/main/content/post/2018-04-11-the-tables-turn-reviewing-yelp/index.Rmd)
In part two I will build on this post and use review text to build a neural network to predict review ratings from the text of the review itself. Sounds like a fun challenge to me! Until next time.